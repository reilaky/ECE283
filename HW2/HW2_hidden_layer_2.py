...
display_step = 10
# BATCH_SIZE = [5, 10, 50]
# HIDDEN_CELLS_1 = [2, 5, 10]
# HIDDEN_CELLS_2 = [2, 5, 10]
# LR = [0.001, 0.005, 0.1, 0.2, 0.5]
# TRAINING_EPOCHS = [5, 50, 100]
...
def init_variables_1_hidden_layer(input_size, num_labels, num_hidden1, num_hidden2):
	...
	# hidden layer 2
	w2 = tf.Variable(tf.truncated_normal([num_hidden1, num_hidden2], stddev=0.1))
	b2 = tf.Variable(tf.constant(1.0, shape = [num_hidden2]))
	# output layer
	w3 = tf.Variable(tf.truncated_normal([num_hidden2, num_labels], stddev=0.1))
	b3 = tf.Variable(tf.constant(1.0, shape = [num_labels]))
...
def model_1_hidden_layer(data, variables):
	...
	layer3_fccd = tf.matmul(layer2_actv, variables['w3']) + variables['b3']
	logits = tf.nn.sigmoid(layer3_fccd)

	return logits

# save_path = '/Users/yankong/Documents/Tensorflow/ECE283_HW2/'
# name = os.path.join(save_path, 'hidden_layer_2.txt')
# file = open(name, 'w')
...
# 			for hidden_cells_2 in HIDDEN_CELLS_2:
# 				for training_epochs in TRAINING_EPOCHS:
# print('LR = {:03f}, epochs:{:d}, Batchsize = {:d}, h1 = {:d}, h2 = {:d}'.format(learning_rate,training_epochs, batch_size, hidden_cells_1, hidden_cells_2))
# file.write('LR = {:03f}, epochs:{:d}, Batchsize = {:d}, h1 = {:d}, h2 = {:d}'.format(learning_rate,training_epochs, batch_size, hidden_cells_1, hidden_cells_2))
with graph.as_default():
	...
	variables = init_variables_1_hidden_layer(INPUT_SIZE, LABELS, hidden_cells_1, hidden_cells_2)
	# initialize model
